...
- **Assignee**: infer from TEAM.md ownership areas — one of Nolan, Adrian, Aiden, Bender, Sara, Karim, Nathan. If genuinely ambiguous, pick the most likely person and state your assumption.
...
- `Assignee` → the person's first name as a **select** (`{"select": {"name": "Adrian"}}`); omit if unassigned
- `Due date` (if provided)
- `Description` (if any)

Do **not** use people-type fields (`Person`/`People`). Then append
...
Do not repeat the full task back. Do not ask for confirmation unless the assignee is genuinely unclear.
...
- **Assignee truly ambiguous**: tag Nolan (<@712636536755060777>) in Discord with a one-liner: "Who's owning X?"